...
update public.courses
set course_category = 'primary_talent'
where course_category = 'primary_tutoring'
  and name ~ '(才藝|美術|繪畫|畫畫|音樂|舞蹈|書法|桌遊|程式|機器人|珠心算)';

alter table public.courses
  alter column course_category set default 'primary_tutoring';
...
create index if not exists courses_course_category_idx
  on public.courses(course_category);
