...
export default function AdminPage() {
  ...
  const fetchTodayVendor = async () => {
    ...
    if (schedule && schedule.vendors) {
      setTodayVendor(Array.isArray(schedule.vendors) ? schedule.vendors[0] : schedule.vendors as any);
    } else {
      setTodayVendor(null);
    }
  };
  ...
  const cancelOrder = async (studentId: string, name: string) => {
    ...
    const today = getToday();
    await supabase.from("orders").delete().eq("student_id", studentId).eq("order_date", today);
    fetchData();
  };
  ...
  const topupStudent = async (studentId: string) => {
    ...
    if (error) { alert("交易紀錄失敗：" + error.message); return; }
  ...
  const renderGradeStats = (orderList: Order[]) => (
    <div className="grid grid-cols-9 gap-3 mt-6">
      {grades.map((grade) => {
        ...
        return (
          <div key={grade} className="bg-blue-700 rounded-xl p-3 text-center">
            <p className="text-sm text-blue-100 font-medium">{grade}</p>
            <p className="text-lg font-bold mt-2">{received} / {total}</p>
            <p className="text-xs text-blue-200 mt-1">未領 {total - received}</p>
          </div>
        );
      })}
    </div>
          ...
          <div className="space-y-2">
            {gradeOrders.map((order) => (
              <div key={order.id} className="flex justify-between items-center bg-white text-black p-4 rounded-xl">
                ...
                {tab === "orders" && (
                  <button onClick={() => cancelOrder(order.student_id, order.name)} className="bg-red-500 text-white px-4 py-2 rounded-lg font-bold">
                    取消
                  </button>
                )}
              </div>
  ...
  const renderStudentSection = (title: string, list: Student[]) => (
    <div className="bg-white rounded-3xl p-6 shadow">
      ...
      {grades.filter((g) => title === "國小部" ? g.includes("小") : g.includes("國")).map((grade) => {
            ...
            {gradeStudents.map((student) => (
              <div key={student.id} className="flex justify-between items-center border-b py-3">
                ...
                <div className="flex gap-2">
                  <button onClick={() => topupStudent(student.id)} className="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded-lg text-sm font-bold">
                    儲值
                  </button>
                  <button onClick={() => deleteStudent(student.id)} className="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded-lg text-sm font-bold">
                    刪除
                  </button>
                </div>
              </div>
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-gray-700 mt-2">訂餐管理後台</p>
            <p className="text-blue-600 font-semibold mt-1">{todayDisplay}</p>
          </div>
          <button onClick={logout} className="bg-red-500 text-white px-4 py-2 rounded-xl font-bold text-sm">
            登出
          </button>
        </div>
        ...
        {tab === "orders" && (
          <div className="bg-slate-900 text-white rounded-3xl p-8">
            <h2 className="text-3xl font-bold">今日訂餐</h2>
            <p className="mt-2 text-gray-300">共 {orders.length} 份</p>
            <div className="mt-4 bg-blue-800 rounded-2xl p-4">
              <p className="font-bold text-lg">今日供餐資訊</p>
            ...
            {orders.filter((o) => !o.received).length > 0 && (
              <div className="mt-8 bg-yellow-400 text-black rounded-2xl overflow-hidden">
                <button onClick={() => setShowUnreceived(!showUnreceived)} className="w-full px-5 py-4 flex justify-between items-center font-bold text-lg">
                  <span>未領名單（{orders.filter((o) => !o.received).length}）</span>
                  <span>{showUnreceived ? "▲" : "▼"}</span>
                </button>
                {showUnreceived && (
                  <div className="px-5 pb-5">
                    <div className="flex flex-wrap gap-3">
                      {orders.filter((o) => !o.received).sort((a, b) => a.grade.localeCompare(b.grade)).map((order) => (
                        <div key={order.id} className="bg-white px-4 py-2 rounded-xl font-semibold">{order.grade}｜{order.name}</div>
                      ))}
                    </div>
                  </div>
                )}
        ...
        {tab === "schedule" && (
          <div className="space-y-6">
            <div className="bg-white rounded-3xl p-6 shadow">
              <h2 className="text-3xl font-bold text-black">本週排餐</h2>
              <p className="text-gray-500 mt-2">安排本週每日供餐內容</p>
            </div>
            <div className="bg-white rounded-3xl p-6 shadow space-y-4">
              {[["mon", "星期一"], ["tue", "星期二"], ["wed", "星期三"], ["thu", "星期四"], ["fri", "星期五"]].map(([key, day]) => (
                <div key={day} className="grid grid-cols-3 gap-4 items-center border-b pb-4">
                  <div className="font-bold text-lg text-black">{day}</div>
                  <select value={weeklySchedule[key]?.vendor_id || ""} onChange={(e) => setWeeklySchedule((prev) => ({ ...prev, [key]: { vendor_id: e.target.value, menu_id: "" } }))} className="border px-4 py-3 rounded-xl text-black">
                    <option value="">選擇商家</option>
                    {vendors.map((vendor) => <option key={vendor.id} value={vendor.id}>{vendor.name}</option>)}
                  </select>
                  <select value={weeklySchedule[key]?.menu_id || ""} onChange={(e) => setWeeklySchedule((prev) => ({ ...prev, [key]: { ...prev[key], menu_id: e.target.value } }))} className="border px-4 py-3 rounded-xl text-black">
              ...
              <button onClick={saveSchedule} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-4 rounded-2xl font-bold">儲存本週排餐</button>
            </div>
          </div>
        )}
        ...
        {tab === "menu" && (
          <div className="space-y-6">
            <div className="bg-white rounded-3xl p-6 shadow">
              <h2 className="text-3xl font-bold text-black">商家管理</h2>
              <p className="text-gray-500 mt-2">管理商家</p>
            </div>
            <div className="bg-white rounded-3xl p-6 shadow">
              <h3 className="text-xl font-bold text-black mb-4">商家管理</h3>
              <div className="flex gap-3 mb-5">
                <input value={vendorName} onChange={(e) => setVendorName(e.target.value)} placeholder="輸入商家名稱" className="flex-1 border px-4 py-3 rounded-xl text-black" />
                <input value={vendorPhone} onChange={(e) => setVendorPhone(e.target.value)} placeholder="商家電話" className="flex-1 border px-4 py-3 rounded-xl text-black" />
                <input value={vendorNote} onChange={(e) => setVendorNote(e.target.value)} placeholder="備註" className="flex-1 border px-4 py-3 rounded-xl text-black" />
                <button onClick={addVendor} className="bg-blue-600 text-white px-5 py-3 rounded-xl font-bold">新增</button>
              </div>
              <div className="space-y-3">
                {vendors.length === 0 ? <p className="text-gray-500">尚未新增商家</p> : vendors.map((vendor) => {
                  const isExpanded = expandedVendor === vendor.id;
                  return (
                    <div key={vendor.id} className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition">
                      <div onClick={() => setExpandedVendor(isExpanded ? "" : vendor.id)} className="flex justify-between items-center cursor-pointer">
                        <div>
                          <p className="font-bold text-xl text-black">{vendor.name}</p>
                          <div className="flex gap-4 mt-2 text-sm text-gray-500">
                            {vendor.phone && <span>📞 {vendor.phone}</span>}
                            <span>🍱 {menus.filter((menu) => menu.vendor_id === vendor.id).length} 道菜</span>
                          </div>
                          {vendor.note && <p className="text-gray-400 mt-1">{vendor.note}</p>}
                        </div>
                        <div className="flex items-center gap-3">
                          <button onClick={(e) => { e.stopPropagation(); deleteVendor(vendor.id); }} className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-xl font-semibold">刪除</button>
                      ...
                      {isExpanded && (
                        <div className="grid md:grid-cols-2 gap-6 mt-6">
                          <div className="bg-gray-50 rounded-2xl p-4">
                            <h4 className="font-bold text-black mb-4">已有菜單</h4>
                            <div className="space-y-3">
                              {menus.filter((menu) => menu.vendor_id === vendor.id).map((menu) => (
                                <div key={menu.id} className="flex justify-between items-center bg-white px-4 py-3 rounded-xl shadow-sm">
                                  <span className="font-medium text-black">{menu.name}</span>
                                  <span className="text-blue-600 font-bold">${menu.price}</span>
                                </div>
                              ))}
                            </div>
                          </div>
                          <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4">
                            <h4 className="font-bold text-blue-900 mb-4">新增菜單</h4>
                            <div className="space-y-3">
                              <input value={menuInputs[vendor.id]?.name || ""} onChange={(e) => setMenuInputs((prev) => ({ ...prev, [vendor.id]: { name: e.target.value, price: prev[vendor.id]?.price || "" } }))} placeholder="輸入菜名" className="w-full border px-4 py-3 rounded-xl text-black" />
                              <input value={menuInputs[vendor.id]?.price || ""} onChange={(e) => setMenuInputs((prev) => ({ ...prev, [vendor.id]: { name: prev[vendor.id]?.name || "", price: e.target.value } }))} placeholder="價格" className="w-full border px-4 py-3 rounded-xl text-black" />
                              <button onClick={() => addMenu(vendor.id)} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-bold">＋ 新增菜單</button>
                            </div>
                          </div>
                        </div>
                      )}
        ...
        {tab === "history" && (
          <div className="bg-slate-900 text-white rounded-3xl p-8">
            <div className="flex justify-between items-center mb-8 bg-slate-800 rounded-2xl p-4">
              ...
              <input type="date" value={historyDate} onChange={(e) => setHistoryDate(e.target.value)} className="bg-white text-slate-900 px-4 py-3 rounded-2xl font-semibold shadow-md border-2 border-blue-200 focus:outline-none focus:ring-4 focus:ring-blue-400" />
            </div>
