...
export default function AttendanceTab() {
  ...
  const fetchData = async () => {
    ...
    // 如果切換到國中模式且尚未選擇課程，自動選擇今天的第一堂課
    if (courseRes.data && courseRes.data.length > 0 && !selectedCourseId) {
      const todayDayOfWeek = new Date().getDay() === 0 ? 7 : new Date().getDay();
      const todaysCourses = courseRes.data.filter(c => c.day_of_week === todayDayOfWeek);
      if (todaysCourses.length > 0) {
        setSelectedCourseId(todaysCourses[0].id);
      } else {
        setSelectedCourseId(courseRes.data[0].id); // 今天沒課就選清單第一個
      }
    }
  ...
  const handleBatchArrive = async (courseId: string | null = null) => {
    ...
    const newLogs = selectedIds.map(id => ({
      student_id: id, date: today, course_id: courseId, status: 'arrived', arrival_time: new Date().toISOString()
    }));

    await supabase.from("attendance_logs").upsert(newLogs, { onConflict: "student_id, date, course_id" });
    alert(`已發送 ${selectedIds.length} 位學生【到班通知】！`);
    setSelectedIds([]); 
  };

  // --- 國中全班下課 ---
  const handleBulkLeaveJunior = async () => {
    // 找出目前這堂課「已到班(arrived)」的學生
    const arrivedIds = courseStudents.filter(s => attendanceLogs.find(l => l.student_id === s.id && l.status === 'arrived')).map(s => s.id);
    
    if (arrivedIds.length === 0) return alert("目前沒有已到班的學生可以下課！");
    if (!confirm(`確定要將這 ${arrivedIds.length} 位學生設為「已離班」並通知家長嗎？`)) return;
    ...
    // 寫入資料庫
    const updatePromises = arrivedIds.map(id => 
  ...
  const saveScores = async () => {
    ...
    if (error) alert("成績儲存失敗：" + error.message);
    else alert("成績已成功儲存！");
  };

  const exportToCSV = () => {
    let csv = "\uFEFF"; // 加入 BOM 讓 Excel 讀取中文不亂碼
    csv += "學生姓名,成績一,成績二\n";
  ...
  // --- 國中名單過濾 (依據課程) ---
  const todayDayOfWeek = new Date().getDay() === 0 ? 7 : new Date().getDay();
  const todaysCourses = courses.filter(c => c.day_of_week === todayDayOfWeek);
            ...
            {loading ? <div className="text-center py-20 text-slate-400 font-bold animate-pulse">資料同步中...</div> : (
              <div className="space-y-4">
                      ...
                      return (
                        <div key={s.id} className="bg-white p-4 rounded-2xl border border-orange-100 shadow-sm flex flex-col gap-3">
                          ...
                          <div className="flex gap-2">
                            ...
                            <button onClick={() => { if(window.confirm(`確定已離班？`)) updateStudentStatus(s.id, 'left'); }} className="flex-1 py-2 bg-slate-800 text-white rounded-xl text-sm font-bold shadow-md hover:bg-slate-700 active:scale-95 transition-all">確認離班</button>
                          </div>
                        </div>
                      );
                    })}
                ...
                <div className="bg-slate-100 p-5 rounded-3xl border border-slate-200">
                  <h3 className="text-sm font-black text-slate-500 mb-2">今日已離班 ({p_left.length})</h3>
                  <div className="flex flex-wrap gap-2">{p_left.map(s => <span key={s.id} className="bg-white px-3 py-1.5 rounded-lg text-xs font-bold text-slate-500">{s.name}</span>)}</div>
                </div>
                <div className="bg-red-50 p-5 rounded-3xl border border-red-100">
                  <h3 className="text-sm font-black text-red-500 mb-2">今日請假 ({p_leave.length})</h3>
                  <div className="flex flex-wrap gap-2">{p_leave.map(s => <span key={s.id} className="bg-white px-3 py-1.5 rounded-lg text-xs font-bold text-red-400">{s.name}</span>)}</div>
                </div>
              </div>
            )}
          </>
        ...
        {systemMode === "junior" && (
          <>
            ...
            <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100 space-y-4">
              <div>
                <label className="block text-slate-500 font-bold mb-2 text-sm">今日課程 (星期{["日","一","二","三","四","五","六"][new Date().getDay()]})：</label>
                <select 
                ...
                >
                  {todaysCourses.length > 0 ? (
                    todaysCourses.map(c => <option key={c.id} value={c.id}>{c.name}</option>)
                  ) : (
                    <option value="">今日無排定課程，請至資料庫新增</option>
                  )}
                  {/* 如果想看別天的課，這裡也可以把全部課程印出來，但用分類隔開 */}
                  {courses.filter(c => c.day_of_week !== todayDayOfWeek).length > 0 && (
                    <optgroup label="--- 其他天課程 ---">
                      {courses.filter(c => c.day_of_week !== todayDayOfWeek).map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                    </optgroup>
                  )}
                </select>
              </div>
                ...
                {juniorTab === "attendance" && (
                  <div className="space-y-4">
                    {courseStudents.length === 0 ? (
                      <div className="text-center py-10 border-2 border-dashed border-slate-200 rounded-3xl text-slate-400 font-bold">
                        此課程尚未綁定任何學生<br/><span className="text-xs">請至 student_courses 資料表新增</span>
                      </div>
                    ) : (
                      <>
                        <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm">
                          <h3 className="text-lg font-black text-slate-800 mb-4 flex items-center gap-2">
                            待簽到 <span className="bg-amber-100 text-amber-700 px-2 py-0.5 rounded-md text-xs">{j_pending.length}</span>
                          </h3>
                          <div className="space-y-3">
                            {j_pending.map(s => {
                              const isChecked = selectedIds.includes(s.id);
                        ...
                        <div className="bg-slate-100 p-5 rounded-3xl border border-slate-200">
                          <h3 className="text-lg font-black text-slate-500 mb-4 flex items-center gap-2">
                            上課中 (已到班) <span className="bg-white text-slate-600 px-2 py-0.5 rounded-md text-xs">{j_arrived.length}</span>
                          </h3>
                        ...
                        <div key={s.id} className="flex items-center justify-between border-b border-slate-50 pb-4">
                          ...
                          <div className="flex gap-2 flex-1">
                            <input 
                              type="number" 
                              placeholder="成績一" 
                              value={currentScores[s.id]?.score_1 || ""}
                              onChange={(e) => handleScoreChange(s.id, "score_1", e.target.value)}
                              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 outline-none focus:border-amber-400 font-bold text-center" 
                            />
                            <input 
                              type="number" 
                              placeholder="成績二" 
                              value={currentScores[s.id]?.score_2 || ""}
                              onChange={(e) => handleScoreChange(s.id, "score_2", e.target.value)}
                              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 outline-none focus:border-amber-400 font-bold text-center" 
                            />
                          </div>
                        </div>
                      ))}
                      {courseStudents.length === 0 && <p className="text-center text-slate-400 py-4 font-bold">尚無學生名單</p>}
                    </div>
